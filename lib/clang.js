...
class Clang {
  ...
  request(methodName, args, callback) {
    const me = this
    ...
    if (arguments.length === 2) {
      ...
      if (!args || typeof args !== 'object') {
        return setImmediate(callback.bind(null, new Error(`args argument must be an object`)))
      }
      requestArgs = Object.assign({
        uuid: me.config.uuid
      }, args)
    } else {
      callback = methodName
      return setImmediate(callback.bind(null, new Error(`request takes 3 arguments: methodName, args (object) and callback function`)))
    }

    if (!requestArgs.uuid) {
      return setImmediate(callback.bind(null, new Error(`uuid missing. Provide it with the 2nd arguments object or with client instantiation. Hint: 'let clang = new Clang({uuid: '12345678', ...})'`)))
    }
    ...
    async.autoInject({
      ...
      request: ['init', (init_data, callback) => {
        ...
        if (!fn) {
          return setImmediate(callback.bind(null, new Error(`Undefined method '${methodName}'`)))
        }
  ...
  send(customer, options, callback) {
    ...
    async.autoInject({
      ...
      upsert: ['get', (record, callback) => {
        ...
        me.request(upsertMethodName, {customer: customer}, (err, result) => {
          if (err) {
            return callback(err)
          }
          if (!result || !result.length) {
            return callback(new Error(`No customer returned after ${upsertMethodName}`))
          }
          if (result.length > 1) {
            return callback(new Error(`Multiple customers returned after ${upsertMethodName}`))
          }
 ...
 */
const normalizeInput = (config, description, clangMethodName, args) => {

  let inputDefinition = description.clangAPI.clangPort[clangMethodName].input
 ...
 */
const normalizeResponse = (config, result) => {
  if (result.msg === true) {
    if (config.debug) {
  ...
  let msgKeys = Object.keys(result.msg)
  if (msgKeys.length === 0) {
    if (config.debug) {
 ...
 */
const buildCallback = (uuid, config, api, pagingArgs, callback) => {
  return apiCallback

  function apiCallback(err, result) {
