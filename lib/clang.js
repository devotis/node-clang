...
class Clang {
  ...
  send(customer, options, callback) {
    ...
    async.autoInject({
      ...
      upsert: ['get', function (record, callback) {
        if (record) {
        ...
        me.request(upsertMethodName, {customer: customer}, function(err, result) {
          if (err) {
            return callback(err);
          }
      ...
      send: ['upsert', function (record, callback) {
        let args = {};
 ...
 */
let normalizeInput = function(config, description, clangMethodName, args) {
  ...
  let moveOptions = function (object, inputDefinition) {
 ...
 */
let normalizeResponse = function(config, result) {
  if (result.msg === true) {
    if (config.debug) {
 ...
 */
let buildCallback = function (uuid, config, api, pagingArgs, callback) {
